...
variable "aws_iam_session_context_current_arn" {
  # count = local.create ? 1 : 0
...
locals {
  ...
  bootstrap_cluster_creator_admin_permissions = { for k, v in {
    cluster_creator = {
      principal_arn = try(var.aws_iam_session_context_current_arn, "")
      type          = "STANDARD"
...
module "kms" {
  ...
  key_administrators        = coalescelist(var.kms_key_administrators, [try(var.aws_iam_session_context_current_arn, "")])
